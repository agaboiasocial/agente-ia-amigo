...
function formatInline(text: string, keyPrefix = ""): ReactNode[] {
  ...
  while (i < text.length) {
    ...
    // inline code `...`
    let m = rest.match(/^`([^`\n]+)`/);
    if (m) { tokens.push({ type: "code", content: m[1] }); i += m[0].length; continue; }

    // link [text](url)
    m = rest.match(/^\[([^\]]+)\]\(([^)\s]+)\)/);
    if (m) { tokens.push({ type: "link", content: m[1], href: m[2] }); i += m[0].length; continue; }

    // raw url
    m = rest.match(/^https?:\/\/[^\s<>()]+/);
    if (m) { tokens.push({ type: "link", content: m[0], href: m[0] }); i += m[0].length; continue; }

    // bold *...*  or **...**
    m = rest.match(/^\*\*([^*\n]+)\*\*/) || rest.match(/^\*([^*\n]+)\*/);
    if (m) { tokens.push({ type: "bold", content: m[1] }); i += m[0].length; continue; }

    // italic _..._
    m = rest.match(/^_([^_\n]+)_/);
    if (m) { tokens.push({ type: "italic", content: m[1] }); i += m[0].length; continue; }

    // strike ~...~
    m = rest.match(/^~([^~\n]+)~/);
    if (m) { tokens.push({ type: "strike", content: m[1] }); i += m[0].length; continue; }
  ...
  return tokens.map((t, idx) => {
    ...
    switch (t.type) {
      case "bold": return <strong key={key}>{t.content}</strong>;
      case "italic": return <em key={key}>{t.content}</em>;
      case "strike": return <span key={key} style={{ textDecoration: "line-through" }}>{t.content}</span>;
      case "code":
        return (
      ...
      default: return <Fragment key={key}>{t.content}</Fragment>;
    }
  });
}
    ...
    if (/^[-*] /.test(line)) {
      ...
      blocks.push(
        <ul key={`b${blocks.length}`} className="list-disc pl-5 my-1 space-y-0.5">
          {buf.map((l, k) => <li key={k}>{formatInline(l, `u${blocks.length}-${k}`)}</li>)}
        </ul>
      );
      continue;
    }
    ...
    if (/^\d+\.\s/.test(line)) {
      ...
      blocks.push(
        <ol key={`b${blocks.length}`} className="list-decimal pl-5 my-1 space-y-0.5">
          {buf.map((l, k) => <li key={k}>{formatInline(l, `o${blocks.length}-${k}`)}</li>)}
        </ol>
      );
      continue;
    }
