...
import {
  RefreshCw, CheckCircle2, XCircle, Loader2, Copy, ArrowRight,
  Smartphone, MessageSquare, Webhook, Wifi, WifiOff,
} from "lucide-react";
...
// ── Proxy helper (same pattern as whatsapp-lead-hub) ──
async function evoProxy(accountId: string, payload: Record<string, unknown>) {
...
function saveInstance(i: Instance | null) {
  if (i) localStorage.setItem(STORAGE_KEY, JSON.stringify(i)); else localStorage.removeItem(STORAGE_KEY);
}
...
function Page() {
  ...
  useEffect(() => { setInstance(loadInstance()); setHydrated(true); }, []);

  if (!hydrated) {
    return <AppLayout title="WhatsApp"><div className="grid place-items-center py-20"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div></AppLayout>;
  }

  if (!instance) {
    return <ConnectFlow accountId={accountId} onConnected={(i) => { saveInstance(i); setInstance(i); }} />;
  }
  return (
    <ManageInstance
      instance={instance}
      onSave={(i) => { saveInstance(i); setInstance(i); toast.success("Configurações salvas"); }}
      onDisconnect={() => {
        if (!confirm("Desconectar WhatsApp?")) return;
        saveInstance(null); setInstance(null); toast.success("WhatsApp desconectado");
      }}
    />
  );
}

/* ── CONNECT FLOW (evolution-proxy based, like lead-hub) ── */

function ConnectFlow({ onConnected, accountId }: { onConnected: (i: Instance) => void; accountId: string | null }) {
  const [state, setState] = useState<"form" | "loading" | "qr" | "connecting" | "error">("form");
  ...
  const handleConnected = useCallback((info: { instanceName: string; profileName: string | null; phoneNumber: string | null }) => {
    cleanup();
    setState("connecting");
    setTimeout(() => {
      const inst: Instance = {
        ...
        autoReceive: true, readReceipts: true,
        team: "Atendimento", defaultAgent: "Sem padrão",
        welcomeMessage: "Olá! 👋 Bem-vindo ao nosso atendimento.",
        awayMessage: "Estamos fora do horário. Retornaremos em breve!",
        schedule: "Comercial (Seg–Sex 09h–18h)",
        sentToday: 0, receivedToday: 0, lastActivity: "agora",
      };
      onConnected(inst);
    }, 900);
  ...
  const startPolling = useCallback((inst: string) => {
    ...
    pollRef.current = window.setInterval(async () => {
      try {
        const data = await evoProxy(accountId, { action: "status", instanceName: inst });
        if (data?.connected) {
          handleConnected({ instanceName: inst, profileName: data.profileName as string, phoneNumber: data.phoneNumber as string });
        }
      } catch { /* ignore */ }
    }, 4000);
  ...
  }, [accountId, handleConnected]);

  // Auto-reconnect when QR expires
  useEffect(() => {
    if (countdown === 0 && qrCode && status !== "open") {
      reconnectRef.current();
    }
  }, [countdown, qrCode, status]);

  // Reconnect (new QR without recreating instance)
  const handleReconnect = useCallback(async () => {
    if (!accountId || !instanceName) return;
    cleanup();
    setQrCode("");
    try {
      // Ensure webhook first
      await evoProxy(accountId, { action: "configure-webhook", instanceName: instanceName.trim() });
      // Get new QR
      const data = await evoProxy(accountId, { path: `/instance/connect/${instanceName.trim()}` });
      const qr = (data?.qrcode as any)?.base64 ?? data?.base64 ?? data?.qr ?? null;
      if (qr) {
  ...
  }, [accountId, instanceName, startPolling]);
  ...
  const startConnect = async () => {
    ...
    setState("loading");
    try {
      const inst = instanceName.trim().replace(/[^a-zA-Z0-9_-]/g, "-");
      const data = await evoProxy(accountId, { action: "setup", instanceName: inst, phoneNumber: phoneNumber.trim() });
      ...
      if (qr) {
        ...
        const statusData = await evoProxy(accountId, { action: "status", instanceName: inst });
        if (statusData?.connected) {
          handleConnected({ instanceName: inst, profileName: statusData.profileName as string, phoneNumber: statusData.phoneNumber as string });
        } else {
          setState("qr");
          startPolling(inst);
          // Try to get QR via connect endpoint
          handleReconnect();
        }
      }

      const verified = data?.webhookVerified as boolean;
      if (verified) {
        toast.success("Instância criada e webhook configurado!");
      } else {
        toast.warning("Instância criada, mas webhook pode não ter persistido. Tente 'Reconfigurar webhook' se necessário.");
      }
    } catch (e: any) {
      setError(e?.message || "Erro ao conectar. Verifique a Evolution API.");
      setState("error");
    }
  };
    ...
    <AppLayout title="Conectar WhatsApp">
      <div className="max-w-xl mx-auto">
        <div className="bg-card rounded-2xl border shadow-sm p-6 md:p-8">
          {state === "form" && (
            <FormView
              instanceName={instanceName} phoneNumber={phoneNumber}
              setInstanceName={setInstanceName} setPhoneNumber={setPhoneNumber}
          ...
          {state === "loading" && (
            <div className="text-center py-10">
              ...
              <p className="text-sm text-muted-foreground mt-4">Criando instância e configurando webhook...</p>
            </div>
          )}
          {state === "qr" && (
            <QrView qrCode={qrCode} countdown={countdown} expired={countdown === 0} onRegenerate={handleReconnect} />
          )}
          {state === "connecting" && <ConnectingView />}
          {state === "error" && <ErrorView message={error} onRetry={startConnect} />}
        </div>
      </div>
    </AppLayout>
  );
...
function FormView({ instanceName, phoneNumber, setInstanceName, setPhoneNumber, onSubmit }: {
    ...
    <div>
      ...
      <div className="space-y-3">
        <label className="block">
          ...
          <input value={instanceName} onChange={(e) => setInstanceName(e.target.value.replace(/[^a-zA-Z0-9_-]/g, ""))}
        ...
        <label className="block">
          ...
          <input value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} placeholder="+55 11 98765-4321"
            className="w-full h-10 px-3 rounded-lg border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-success/40" />
        </label>
...
function QrView({ qrCode, countdown, expired, onRegenerate }: { qrCode: string; countdown: number; expired: boolean; onRegenerate: () => void }) {
  const src = qrCode || "";
  return (
    <div className="text-center">
      ...
      {!expired && (
        <div className="text-xs text-muted-foreground mt-2">
          Expira em {String(Math.floor(countdown / 60)).padStart(2, "0")}:{String(countdown % 60).padStart(2, "0")}
          {" · "}
          <button onClick={onRegenerate} className="text-success hover:underline">Gerar novo QR</button>
        </div>
      )}

      <ol className="mt-6 space-y-2 text-left max-w-sm mx-auto">
        {["Abra o WhatsApp no seu celular", "Toque em Menu (⋮) ou Configurações", 'Toque em "Dispositivos conectados"', 'Toque em "Conectar dispositivo"', "Aponte a câmera para o QR Code"].map((t, i) => (
          <li key={i} className="flex items-start gap-3 text-sm">
            <span className="h-6 w-6 rounded-full bg-success/15 text-success grid place-items-center text-xs font-bold shrink-0">{i + 1}</span>
...
function ErrorView({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <div className="text-center py-6">
      <div className="h-14 w-14 rounded-full bg-destructive/15 text-destructive grid place-items-center mx-auto"><XCircle className="h-8 w-8" /></div>
      ...
      <button onClick={onRetry} className="mt-5 h-10 px-5 rounded-lg bg-success text-success-foreground text-sm font-semibold inline-flex items-center gap-1.5">
...
function ManageInstance({ instance, onSave, onDisconnect }: { instance: Instance; onSave: (i: Instance) => void; onDisconnect: () => void }) {
  const [form, setForm] = useState<Instance>(instance);
  const set = <K extends keyof Instance>(k: K, v: Instance[K]) => setForm((p) => ({ ...p, [k]: v }));
  const copyWebhook = () => { navigator.clipboard.writeText(form.webhookUrl); toast.success("URL copiada"); };

  return (
    <AppLayout title={form.inboxName} actions={
      <div className="flex items-center gap-2">
        <span className="text-[11px] px-2 py-1 rounded-full font-semibold inline-flex items-center gap-1 bg-success text-success-foreground">
          <Wifi className="h-3 w-3" /> Conectado
        </span>
        <button onClick={onDisconnect} className="h-9 px-3 rounded-lg border border-destructive text-destructive text-xs font-semibold hover:bg-destructive/5">Desconectar</button>
      </div>
    }>
      <div className="space-y-6">
        <div className="bg-card border rounded-xl p-4 md:p-5 flex flex-wrap sm:flex-nowrap items-center gap-3 md:gap-4">
          <div className="h-12 w-12 rounded-full bg-success/15 text-success grid place-items-center shrink-0"><CheckCircle2 className="h-7 w-7" /></div>
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-brand">WhatsApp Conectado!</div>
            <div className="text-xs text-muted-foreground truncate">{form.profileName} · {form.phone}</div>
          </div>
          <Link to="/conversas" className="h-9 px-4 rounded-lg bg-success text-success-foreground text-sm font-semibold inline-flex items-center gap-1.5 shrink-0">
        ...
        <div className="bg-card border rounded-xl p-5 space-y-4">
          ...
          <Field label="Nome da caixa de entrada">
            <input value={form.inboxName} onChange={(e) => set("inboxName", e.target.value)} className="w-full h-10 px-3 rounded-lg border bg-background text-sm" />
          </Field>
        ...
        <div className="flex justify-end">
          <button onClick={() => onSave(form)} className="h-10 px-5 rounded-lg bg-success text-success-foreground text-sm font-semibold hover:opacity-95">Salvar configurações</button>
        </div>
      </div>
    </AppLayout>
  );
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <label className="block"><span className="text-xs font-medium block mb-1">{label}</span>{children}</label>;
}
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: () => void }) {
  return (
    <div className="flex items-center justify-between gap-3 p-3 rounded-lg border bg-background">
      <span className="text-sm font-medium">{label}</span>
      <button type="button" onClick={onChange} className={`relative h-5 w-9 rounded-full transition-colors ${checked ? "bg-success" : "bg-muted-foreground/30"}`}>
        <span className={`absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-all ${checked ? "left-[18px]" : "left-0.5"}`} />
      </button>
    </div>
  );
}
function InfoCard({ label, value, icon: Icon, tone }: { label: string; value: string; icon: React.ComponentType<{ className?: string }>; tone?: "success" | "brand" }) {
  const cls = tone === "success" ? "text-success" : tone === "brand" ? "text-brand" : "text-foreground";
  return (
    <div className="bg-card border rounded-xl p-3">
      <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground"><Icon className="h-3.5 w-3.5" /> {label}</div>
      <div className={`text-sm font-semibold mt-1 truncate ${cls}`}>{value}</div>
    </div>
  );
}
